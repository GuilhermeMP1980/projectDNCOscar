...
# ============================
# 1. Geração de cenários simulados
# ============================
def gerar_cenario_simulado(tipo: str = "normal", n_estoques: int = 5, n_skus: int = 10):
    ...
    for sku in skus:
        if tipo == "normal":
            vinculados = random.sample(estoques, random.randint(2, min(3, n_estoques)))
        elif tipo == "suspeito":
            vinculados = random.sample(estoques, 1)
        else:
            raise ValueError("Tipo inválido. Use 'normal' ou 'suspeito'.")
...
if __name__ == "__main__":
    ...
    estoques_n, skus_n, arestas_n = gerar_cenario_simulado("normal", 5, 10)
    G_normal = construir_grafo(estoques_n, skus_n, arestas_n)
    suspeitos_n = casos_suspeitos(G_normal, skus_n, pesos)
    for s in suspeitos_n:
        print(f"{s['sku']} → Similaridade média: {s['media_similaridade']}, Estoques: {s['estoques_vinculados']}")

    print("\n🚨 Cenário SUSPEITO")
    estoques_s, skus_s, arestas_s = gerar_cenario_simulado("suspeito", 5, 10)
    G_suspeito = construir_grafo(estoques_s, skus_s, arestas_s)
    suspeitos_s = casos_suspeitos(G_suspeito, skus_s, pesos)
    for s in suspeitos_s:
        print(f"{s['sku']} → Similaridade média: {s['media_similaridade']}, Estoques: {s['estoques_vinculados']}")
